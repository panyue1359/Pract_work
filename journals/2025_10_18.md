...
- 将VSCode插件文件夹剪切到其他位置，复制地址，管理员权限打开cmd命令行窗口，执行`mklink /D "插件文件夹原始位置" "插件文件夹新位置"`，如`mklink /D "C:\Users\Fan_Sir\.vscode" "E:\.vscode\extensions"`
  logseq.order-list-type:: number
- #+BEGIN_PINNED
  移动腾讯系软件数据（微信、QQ、企业微信）
  #+END_PINNED
- 腾讯软件数据默认存储位置为“C:\\Users\\DOSky\\AppData\\Roaming\\Tencent”（这个路径不是安装路径或聊天文件位置）[清理C盘的方法，就是欺骗它_roaming tencent-CSDN博客](https://blog.csdn.net/CQZHOUZR/article/details/144467150)
- 将Tencent文件夹剪切到其他位置，复制地址，管理员打开cmd窗口，执行`mklink /J "Tencent文件夹原始路径" "Tencent文件夹新路径"`，如`mklink /J "C:\Users\DOSky\AppData\Roaming\Tencent" "D:\Data\Tencent"`
  logseq.order-list-type:: number
- ## Podman安装及使用
  collapsed:: true
