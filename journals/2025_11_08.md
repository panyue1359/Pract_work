...
- 执行`apptainer shell apptainer镜像路径`创建容器
  logseq.order-list-type:: number
	- apptainer创建容器需要管理员安装`set-uid`或者启动namespace功能，CentOS 7.4系统的namespace功能默认关闭，需要在管理员用户执行`echo 10000 > /proc/sys/user/max_user_namespaces`，以开启该功能 [CentOS 7 启用 user namespaces（用户命名空间） - 操作系统 - 123si 博客](https://www.123si.org/os/article/centos-7-enable-user-namespaces/)
- #+BEGIN_WARNING
  问题
  #+END_WARNING
